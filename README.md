...
**取消边界**：请求取消（客户端关闭页面/连接、请求上下文结束）与领域提交、响应
交付、排空租约是三个确定边界。所有重计算——拓扑解析（JSON 解码）、Dinic 最大流、
多源 Dijkstra——都观察请求上下文：调用方消失后，正在进行的解析或求解在下一个检查
点立即中止并释放其租约，**不会**为已经没有调用方的请求跑完大图、也不会把排空拖过
`DRAIN_TIMEOUT`。尚未提交的请求（被取消的 `/mincut`、被取消的 `/incidents`
创建）不改变任何仓储；只有携带新编号的创建响应**成功写出之后**事件才入库，响应写
入失败不会留下调用方无法命名的孤立事件。
...
- 推进到**当前分钟**是幂等重试：返回空 `new_arrivals` 与相同快照（终态后同样
  适用，仍是 200）；
- **交付边界（at-least-once 增量）**：时钟、到达增量与状态在领域层一次性提交，
  提交不可回滚。如果携带非空增量的 200 响应因编码或连接写入失败而**没有交付**，
  该增量被记为未交付；此后**第一个同分钟请求重放同一批增量与同一快照**（而不是空
  `new_arrivals`），这批增量被成功交付后标记才清除，因此"提交已成功、响应丢失、
  同分钟重试却返回空"不会再让调用方永久漏记。由其他并发同分钟客户端成功收到的空
  幂等重试不会清除该标记；
...
- **取消 / 交付边界**：单元验收以可取消上下文、失败响应写入器与排空屏障固定交错
  顺序——大图解析/最短路/Dinic 在请求取消后立即中止且租约归零；创建响应写失败不
  入库（事件数量为 0、无孤立事件）；推进已提交而响应写失败后，同分钟重放同一批
  `new_arrivals` 与同一快照，成功交付后才回到空幂等重试（时钟、状态、增量不变）；
  黑盒验收额外复现"客户端关闭完整请求后立即 `SIGTERM`"：废弃计算不再阻塞排空，
  进程在 `DRAIN_TIMEOUT` 内归零并以 `0` 退出；
